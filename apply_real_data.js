...
export default function Home() {
  ...
  const winnerStats = useMemo(() => {
    ...
    return Object.keys(counts).map(code => {
      ...
      return { code, name: team ? team.name : code, votes: counts[code], pct: Number(((counts[code] / total) * 100).toFixed(1)) };
...
`;
content = content.replace('export default function Home() {', hookCode);

// 2. Replace 1.2 million
...
// 4. Featured Match Card (Lines ~100-138)
...
// 6. Live Ticker
content = content.replace('{ icon: "groups", label: "Live Predictions", value: "1,242,509", color: "#38bdf8" }', '{ icon: "groups", label: "Live Predictions", value: totalPredictions.toString(), color: "#38bdf8" }');
content = content.replace('{ icon: "star", label: "Top Predictor", value: "Tactical_Titan", color: "#f59e0b" }', '{ icon: "star", label: "Top Predictor", value: topPredictorName, color: "#f59e0b" }');
content = content.replace('{ icon: "emoji_events", label: "Prize Pool", value: "$500,000", color: "#fbbf24" }', '{ icon: "emoji_events", label: "Community", value: "Real-time voting", color: "#fbbf24" }');

// 7. By the Numbers Stats section
content = content.replace('{ value: "1.2M+", label: "Active Predictors"', '{ value: totalUsers.toString(), label: "Active Predictors"');
content = content.replace('{ value: "$500K", label: "Prize Pool"', '{ value: globalVoteCount.toString(), label: "Global Votes"');

// 8. Featured Stars
content = content.replace('title: "Tactical Titan"', 'title: topPredictorName || "Community Legend"');

// 9. Top Predictions section
const topPredictionsRegex = /<div className="lg:col-span-2 space-y-3">[\s\S]*?<\/div>\n          <\/div>\n        <\/div>\n      <\/section>/;
const newTopPredictions = `<div className="lg:col-span-2 space-y-3">
              {winnerStats.length === 0 ? (
                <p className="text-slate-400">No predictions made yet. Be the first!</p>
              ) : (
                winnerStats.slice(0, 5).map((c, i) => {
...
content = content.replace(topPredictionsRegex, newTopPredictions);
